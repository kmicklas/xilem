...
impl<T: Selectable> TextWithSelection<T> {
    ...
    pub fn accessibility(&mut self, ctx: &mut AccessCtx, parent_node: &mut NodeBuilder) {
        ...
        if let Some(selection) = &self.selection {
            let anchor_affinity = if selection.anchor == selection.active {
                selection.active_affinity
            } else {
                Affinity::Downstream
            };
            let anchor =
                self.access_position_from_offset(parent_node, selection.anchor, anchor_affinity);
    ...
    pub fn set_selection_from_access_event(&mut self, event: &AccessEvent) -> bool {
        ...
        self.selection = Some(Selection::new(anchor, active, active_affinity));
        self.needs_selection_update = true;
        true
    }
}
